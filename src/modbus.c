...
/* Write a value to the specified register of the remote device.
   Used by write_bit and write_register */
static int write_single_no_timeout(modbus_t *ctx, int function, int addr, int value)
{
    int rc;
    int req_length;
    uint8_t req[_MIN_REQ_LENGTH];
...
/* Writes a value in one register of the remote device */
int modbus_write_register_broadcast(modbus_t *ctx, int addr, int value)
{
    if (ctx == NULL) {
        errno = EINVAL;
        return -1;
    ...
    return write_single_no_timeout(ctx, MODBUS_FC_WRITE_SINGLE_REGISTER, addr, value);
}
...
/* Write the values from the array to the registers of the remote device */
int modbus_write_registers_broadcast(modbus_t *ctx, int addr, int nb, const uint16_t *src)
{
    int rc;
    int i;
    int req_length;
